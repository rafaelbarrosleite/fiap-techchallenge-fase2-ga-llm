...
| D4 | Contagens de testes | Documentos citam 58, 79 e 113 testes | Cada número pertence ao encerramento de uma missão distinta | Não há divergência matemática. O relatório final identifica os números como snapshots; o manifesto da entrega registra a contagem final. |
...
Os intervalos e deltas narrados coincidem com `uncertainty_results.json`. O provider oficial da Missão 5 é `fake`, factualidade e segurança estão aprovadas, e `individual_data_sent=false` no manifesto LLM.
