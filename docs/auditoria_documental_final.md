...
| D4 | Contagens de testes | Documentos históricos citam 58, 79, 113, 120 e 161 testes | Cada número pertence ao encerramento de uma missão distinta | Não há divergência matemática. O relatório final identifica os números como snapshots; o manifesto consolidado atual registra 182 testes. |
...
| D10 | Requisito LLM individual | A interpretação conservadora anterior limitava a LLM a agregados | Contrato individual 3.0, exemplo versionado e artefatos fake/OpenAI | A lacuna foi resolvida sem alterar evidência histórica: o caso vem do desenvolvimento e é reduzido a sinais não reconstruíveis antes do provider. |
...
Os intervalos e deltas narrados coincidem com `uncertainty_results.json`. O provider oficial da Missão 5 é `fake`, factualidade e segurança estão aprovadas, e `individual_data_sent=false` no manifesto LLM agregado. A evidência complementar V4 registra uma chamada real agregada, 327/327 fatos e avaliação científica não aprovada. A extensão individual usa uma representação autorizada de desenvolvimento sem linha bruta, passou 40/40 fatos e seis dimensões no fake e na OpenAI real.
