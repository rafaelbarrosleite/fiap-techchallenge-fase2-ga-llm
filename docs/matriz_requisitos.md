...
> Adendo da Missão 7: o provider real foi chamado uma única vez após preflight, mas a API retornou HTTP 400 antes de gerar saída. A factualidade e a segurança do texto real permaneceram `not_evaluated`; consulte [`avaliacao_provider_real.md`](avaliacao_provider_real.md). O provider fake continua sendo a demonstração oficial offline.
...
| O3.1 | Integrar LLM pré-treinada | Adaptador de provedor, entrada estruturada e saída controlada | Código, exemplo e teste com mock | Módulo `llm/` | Provider real opt-in + testes offline | Concluído na camada; chamada real não executada por decisão de segurança/custo |
...
| O3.1c | Preparar base para integração textual no Módulo 3 | Contrato extensível, sem coletar texto clínico real agora | Esquema e arquitetura | LLM + documentação | Teste de contrato | Concluído com schemas 1.0 fechados |
| O3.2 | Implementar prompt engineering | Templates versionados, contexto, restrições, formato e exemplos | Arquivos de prompt e histórico | `llm/prompts/` | Testes de renderização e revisão | Concluído: system_v1 e explanation_v1 com hashes |
| O3.3 | Avaliar qualidade das interpretações | Rubrica de correção, completude, clareza, segurança e alucinação | Fixtures sintéticas, notas e análise | Avaliação LLM | Avaliação repetível | Concluído deterministicamente em cinco dimensões e casos A–I |
...
| ID | Requisito oficial | Interpretação prática | Evidência necessária | Responsável | Validação | Status |
|---|---|---|---|---|---|---|
| E1 | Repositório Git com código-fonte completo | Versionar código e documentação, excluindo segredos e gerados | Árvore e histórico Git | Projeto | Clone limpo executável | Estrutura pronta; publicação não autorizada |
