...
- A identidade da execução mock V1 inclui a assinatura do código que a produziu. A adição posterior do contrato V2 ao pacote `llm/` alterou essa assinatura, e o engine passou a recusar o reaproveitamento do artefato congelado com `ManualInterventionRequired`. A execução permanece reproduzível — reexecutá-la offline devolve conteúdo byte a byte idêntico, incluindo as 139 verificações factuais e a nota `1,0` —, mas o artefato não foi regenerado nem re-selado: fazê-lo invalidaria o selo do contrato V2, que congela a árvore de hashes de `artifacts/llm_evaluation` e é precondição do preflight da avaliação real V4, cuja resposta OpenAI é de chamada única e não pode ser reproduzida. Optou-se por preservar a cadeia e documentar a limitação em vez de reescrever evidência para satisfazer uma verificação.
